...
#'  and 1 indicates the most stable (measured distance is much less than predicted).
#'
...
calcStability <- function(predicted.dist, measured.dist){
  tryCatch({
    ...
    if (predicted.dist < 0 || predicted.dist > 1) {
      stop("predicted.dist should be in range [0, 1]")
    }
    if (measured.dist < 0 || measured.dist > 1) {
      stop("measured.dist should be in range [0, 1]")
    }
  }, error = function(e) {
