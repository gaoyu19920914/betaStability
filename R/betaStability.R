...
#' @param geo_enabled Whether to include geographic info for gdmPred (default: TRUE)
#' @param GAM.dist.method The method for calculating dist for gamPred (default: "manhattan")
#' @param vegdist.method The method for calculating comdist from comtable (default: "bray")
#' @param seed The random seed for reproducibility of rfPred and xgboostPred (default: 42)
#'
#' @returns If method length is 1, returns a column vector of predicted stability values.
#'   If method length > 1, returns a data frame with each column representing results
#'   from each selected method.
#'
#' @examples
#' data(varespec)
#' data(varechem)
...
betaStability <- function(comtable = NULL,
  ...
  valid_methods <- c("linearPred", "mlPred", "glmPred", "gamPred",
  ...
  if (is.null(envdist)) {
    envdist <- dist(BBmisc::normalize(envmeta,
                                       method = "range",
                                       margin = 2),
                     method = "euclidean")
  }
  ...
  call_method <- function(m) {
    result <- switch(m,
      "linearPred" = {
        linearPred(comdist = comdist, envdist = envdist, sitenames = sitenames)
      },
      "mlPred" = {
        mlPred(comdist = comdist, envmeta = envmeta, sitenames = sitenames)
      },
      "glmPred" = {
        glmPred(comdist = comdist, envmeta = envmeta, sitenames = sitenames)
      },
      "gamPred" = {
        gamPred(comtable = comtable, envmeta = envmeta, comdist = comdist,
                sitenames = sitenames, GAM.dist.method = GAM.dist.method)
      },
      "gdmPred" = {
        gdmPred(comdist = comdist, envmeta = envmeta, sitenames = sitenames,
                X = X, Y = Y, geo_enabled = geo_enabled)
      },
      "rfPred" = {
        rfPred(comdist = comdist, envmeta = envmeta, sitenames = sitenames, seed = seed)
      },
      "xgboostPred" = {
        xgboostPred(comdist = comdist, envmeta = envmeta, sitenames = sitenames, seed = seed)
      }
    )
    return(result)
  }
